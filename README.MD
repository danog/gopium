<p align="center">
    <img src="https://raw.githubusercontent.com/danog/gopium/master/gopher.png" alt="gopium"/>
</p>

# Gopium 🌺: Smart Go Structures Optimizer and Manager

[![lint](https://github.com/danog/gopium/workflows/lint/badge.svg)](https://github.com/danog/gopium/actions?query=workflow%3Alint+branch%3Amaster+)
[![build](https://github.com/danog/gopium/workflows/build/badge.svg)](https://github.com/danog/gopium/actions?query=workflow%3Abuild+branch%3Amaster+)
[![test](https://github.com/danog/gopium/workflows/test/badge.svg)](https://github.com/danog/gopium/actions?query=workflow%3Atest+branch%3Amaster+)
[![report](https://goreportcard.com/badge/github.com/danog/gopium?nocache=1)](https://goreportcard.com/report/github.com/danog/gopium)
[![version](https://img.shields.io/github/go-mod/go-version/danog/gopium)](https://github.com/danog/gopium/blob/master/go.mod)
[![license](https://img.shields.io/github/license/danog/gopium)](LICENSE)
...
To install Gopium VSCode Extension use [vscode marketplace](https://marketplace.visualstudio.com/items?itemName=danog.gopium).

To install and update Gopium CLI use:

```bash
go install github.com/danog/gopium@latest
```
...
```bash
# exec transformations against all structs starting A inside danog/gopium/gopium package
# and print results formatted as json array to standart output
gopium -r ^A go_std danog/gopium/gopium filter_pads memory_pack separate_padding_cpu_l1_top separate_padding_cpu_l1_bottom
```
...
```bash
# exec transformations against all structs inside danog/gopium/gopium package
# with target arm architecture and preset cache lines 8,8,8
# and write results to go package danog/gopium/gopium copy
gopium -a arm -l 8 8 8 ast_go_tree danog/gopium/gopium false_sharing_cpu_l1
```
...
```bash
###
# creates new folder `transaction_gopium` inside /Users/danog/proj/src/danog/gopium/examples/
#
...
#
# -p defines full path to package `danog/gopium/examples/transaction`
# it could be omitted if full package name would be used instead `gopium ast_go_tree danog/gopium/examples/transaction memory_pack ...`
###
gopium ast_go_tree transaction memory_pack struct_annotate_comment add_tag_group_force -p /Users/danog/proj/src/danog/gopium/examples/transaction
```
...
# it could be omitted if full package name would be used instead `gopium ast_go_tree danog/gopium/examples/transaction memory_pack ...`
###
gopium ast_go_tree transaction filter_pads false_sharing_cpu_l1 add_tag_group_force -p /Users/danog/proj/src/danog/gopium/examples/transaction
```
...
###
gopium -l 64 -l 64 -l 64 ast_go_tree transaction filter_pads explicit_paddings_system_alignment cache_rounding_cpu_l1_discrete struct_annotate_comment add_tag_group_force -p /Users/danog/proj/src/danog/gopium/examples/transaction
```
...
```go
// pkg: danog/gopium/examples/transaction
...
**simple business transaction aggregator (`gopium -r transaction ast_go_tree danog/gopium/examples/transaction memory_pack`)**

```go
// pkg: danog/gopium/examples/memory_pack/transaction
...
**simple business transaction aggregator (`gopium -r aggregate ast_go_tree danog/gopium/examples/transaction filter_pads false_sharing_cpu_l1`)**

```go
// pkg: danog/gopium/examples/false_sharing_cpu_l1/transaction
...
**simple business transaction aggregator (`gopium ast_go_tree danog/gopium/examples/transaction filter_pads explicit_paddings_system_alignment cache_rounding_cpu_l1_discrete`)**

```go
// pkg: danog/gopium/examples/cache_rounding_cpu_l1_discrete/transaction
...
See [examples](examples) and [godoc](https://pkg.go.dev/github.com/danog/gopium?tab=doc) for more examples, benchmarks details and godoc.  
